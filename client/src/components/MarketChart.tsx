...
export function MarketChart({ data }: { data: ChartPoint[] }) {
  ...
  if (data.length < 2) {
    return (
      <div className="relative min-h-[250px] overflow-hidden border-y border-[var(--rule)] bg-[var(--surface-raised)] px-6 py-6">
        ...
        <div className="relative grid min-h-[202px] grid-cols-[1fr_auto] items-end gap-5">
          <div className="self-center">
            ...
            <p className="mt-3 max-w-md text-sm leading-relaxed text-[var(--ink-soft)]">The plot only renders returned research data; it never uses placeholder market movement.</p>
          </div>
          <div className="space-y-2 border-l border-[var(--rule)] pl-4 font-mono text-[11px] uppercase tracking-[0.1em] text-[var(--ink-faint)]"><p>Period · —</p><p>Points · 00</p><p>Source · pending</p></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[270px] w-full border-y border-[var(--rule)] bg-[var(--surface-raised)] py-5 pr-3 sm:h-[300px]">
