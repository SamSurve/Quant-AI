/** Evidence Terminal Continuum reminder: the Research Desk remains a secondary evidence tool, with spacious readable prompts and responses that preserve the existing onSend flow. */
...
export function ChatPanel({
  ...
  return (
    <section className="ledger-panel research-desk-panel flex min-h-[600px] flex-col overflow-hidden" aria-label="Financial AI chat">
      <div className="relative flex items-center justify-between border-b border-[var(--rule)] px-6 py-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <h2 className="mt-0.5 font-serif text-xl tracking-[-0.03em] text-[var(--ink)]">Research desk exchange</h2>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between border-b border-[var(--rule)] bg-[var(--surface-raised)] px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--ink-faint)]"><span>Session ledger</span><span>{messages.length ? `${Math.ceil(messages.length / 2)} research turn${messages.length > 2 ? "s" : ""}` : "No active brief"}</span></div>
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6">
        {messages.length === 0 ? (
          <div className="border border-[var(--rule)] bg-[var(--surface-raised)] p-4">
            ...
            <p className="text-base font-medium">Open a research ledger with QuantAI.</p>
            </div>
            <p className="mt-2 text-sm leading-relaxed text-[var(--ink-soft)]">Frame an analyst action: pressure-test a thesis, trace a source lane, or request a sourced market update.</p>
          </div>
        ) : (
      ...
      <div className="border-t border-[var(--rule)] bg-[var(--surface-raised)] p-5">
        <div className="mb-3"><p className="ledger-label mb-2">Research prompts</p><div className="flex gap-2 overflow-x-auto pb-1">
          {prompts.map((prompt) => (
            <button key={prompt} type="button" onClick={() => submit(prompt)} disabled={disabled || isLoading} className="whitespace-nowrap border-b border-[var(--rule-strong)] pb-1 text-xs font-semibold text-[var(--ink-soft)] transition-colors hover:border-[var(--research-indigo)] hover:text-[var(--research-indigo)] disabled:opacity-40">
              {prompt}
            </button>
          ))}</div></div>
        <div className="flex items-end gap-2 rounded-sm border border-[var(--rule-strong)] bg-[var(--surface)] p-1.5 focus-within:border-[var(--research-indigo)]">
          <textarea
            ...
            rows={2}
            className="min-h-[48px] flex-1 resize-none bg-transparent px-2 py-1 text-[15px] leading-relaxed text-[var(--ink)] outline-none placeholder:text-[var(--ink-faint)] disabled:cursor-not-allowed"
          />
        ...
        </div><p className="mt-2.5 text-[11px] leading-relaxed text-[var(--ink-faint)]">Enter to send · Shift + Enter for a new line · trace factual values to the source lane before acting.</p>
      </div>
    </section>
  );
}
